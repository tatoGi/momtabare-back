...
class BogPaymentController extends Controller
{
    ...
    public function saveCardForAutomaticPayments(Request $request, $orderId)
    {
        ...
        if (!method_exists($this->bogPayment, 'saveCardForAutomaticPayments')) {
            return response()->json(
                [
                    'success' => false,
                    'message' => 'Method saveCardForAutomaticPayments not implemented in BogPayment service',
                ],
                500,
            );
        }
    ...
    public function confirmPreAuthorization(Request $request, $orderId)
    {
        ...
        if (! method_exists($this->bogPayment, 'confirmPreAuthorization')) {
            return response()->json(
                [
                    'success' => false,
                    'message' => 'BOG Payment method confirmPreAuthorization not implemented',
                ],
                500
            );
        }
