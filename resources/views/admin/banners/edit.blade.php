                    ...
                    <div class="p-6">
                        ...
                        @foreach (config('app.locales') as $locale)
                            <div id="locale-{{ $locale }}"
                                class="locale-content {{ $locale !== app()->getLocale() ? 'hidden' : '' }} space-y-4">
                                ...
                                <div>
                                    <label for="slug_{{ $locale }}"
                                        class="block text-sm font-semibold text-gray-700 mb-2">
                                        <span class="text-red-500">*</span> URL Keyword
                                ...
                                <div>
                                    ...
                                    <textarea id="description_{{ $locale }}" name="{{ $locale }}[desc]" rows="6"
                            ...
                            <select id="typeselect" name="type_id"
                                ...
                                @foreach ($bannerTypes as $key => $type)
                                    <option value="{{ $type['id'] }}"
                                        {{ $type['id'] == $banner->type_id ? 'selected' : '' }}>
                                        {{ __('bannerTypes.' . $key) }}
                                    </option>
                                @endforeach
                            </select>
                            <svg class="absolute right-3 top-3.5 w-5 h-5 text-gray-400 pointer-events-none"
        ...
        $(document).ready(function() {
            ...
            $('.language-tab').on('click', function(e) {
                ...
                $('.language-tab').each(function() {
                    $(this).attr('class',
                        'language-tab flex items-center gap-2 px-4 py-2 rounded-lg font-semibold transition-all duration-300 bg-gray-100 text-gray-700 hover:bg-gray-200'
                        );
                });

                $(this).attr('class',
                    'language-tab flex items-center gap-2 px-4 py-2 rounded-lg font-semibold transition-all duration-300 bg-gradient-to-r from-blue-500 to-blue-600 text-white shadow-md'
                    );
