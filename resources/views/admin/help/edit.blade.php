<x-admin.admin-layout>
    <form action="{{ route('help.update', [app()->getlocale()]) }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200">
            <div class="bg-gradient-to-r from-blue-500 to-blue-600 px-6 py-4">
                <h2 class="text-xl font-bold text-white flex items-center gap-2">
                    {{ __('admin.help') }} (Translations)
                </h2>
            </div>
            <div class="p-6">
                    ...
                    <div id="locale-{{ $locale }}" class="locale-content {{ $locale !== app()->getLocale() ? 'hidden' : '' }} space-y-4">
                        <div>
                            ...
                            <textarea id="description_{{ $locale }}"
                                      ...
                                      class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all @error('text_' . $locale) border-red-500 @enderror">{{ old('text_' . $locale, $help->{'text_' . $locale} ?? '') }}</textarea>
                            @error('text_' . $locale)
