...
<div class="flex flex-wrap my-5 -mx-2">
    <div class="w-full bg-white mt-2 mb-2 rounded-md p-4 mx-auto">
        ...
        <form action="{{ route('pages.update', [app()->getlocale() , $page->id]) }}" method="POST" class="max-w-md mx-auto" enctype="multipart/form-data">
            @csrf
            @method('PUT') 
            <!-- page_id -->
            ...
            <div id="tab-contents">
                @foreach (config('app.locales') as $locale)
                    <div id="locale-{{ $locale }}"
                        ...
                        <div class="mb-4">
                            <label for="keywords" class="block font-medium text-gray-700">Keywords ( {{ __('admin.locale_' . $locale) }})</label>
                            <input type="text" value="{{$page->translate($locale)->keywords ?? '' }}"  id="keywords_{{ $locale }}" 
                             name="{{ $locale }}[keywords]"  class="border-gray-300 py-2
                              focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border rounded-md shadow-sm p-2">
                            <p class="text-sm text-gray-500">Enter keywords separated by commas (e.g., keyword1, keyword2, keyword3)</p>
                            @error('keywords')
                                <span class="text-red-500 text-sm">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="flex w-full items-center justify-center flex-col mb-2">
                            ...
                            <textarea id="description_{{ $locale }}" name="{{ $locale }}[desc]" rows="4"
                                ...
                                placeholder="Description">{{$page->translate($locale)->desc ?? '' }}</textarea>
            ...
            <div class="flex s justify-center items-center flex-col mb-2">
                <label for="type" class="text-sm font-medium text-gray-900 dark:text-gray-400">{{ trans('admin.type') }}</label>
            ...
            <div class="flex justify-between">
                <div class="mb-4">
                    <button type="submit" class="bg-indigo-500 text-white py-2 px-4 rounded hover:bg-indigo-600">Update Page</button>
                </div>
                <div>
                    <a href="/{{app()->getlocale()}}/admin/products" class="bg-indigo-500 text-white py-2 px-4 rounded hover:bg-indigo-600">Back</a>
                </div>
            </div>
